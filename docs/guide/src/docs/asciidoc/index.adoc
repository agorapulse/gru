...
=== Integration Tests
You use `Http` client to verify controller within `@Integration` test. Once `gru-spring-integration-test` is on the classpath then you can inject `Gru<Http>` bean into your tests.
...
<2> Inject `Gru<Http>` into your tests
