...
* `gru-grails` has been removed. Grails 6 is not compatible with Gradle 9, and migrating to Apache Grails 7+ requires Groovy 4 + a series of breaking API changes that would force a separate major-version release of the module. Consumers on Grails should pin to gru `2.x` until a dedicated Apache-Grails port lands.
